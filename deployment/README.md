...
| Component | File | Platform | Runs |
|---|---|---|---|
| Trading Bot | `bot-ironclad.js` | **Railway** | Every 15 min, 24/7 |
| Railway Runner | `railway-runner.js` | **Railway** | Persistent process — wraps the trading bot |
| Research Bot | `research.js` | GitHub Actions | 9am + 6pm UK daily |
| Fund Monitor | `monitor.js` | GitHub Actions | With research runs |
| Dashboard | `docs/index.html` | GitHub Pages | Auto-generated by research bot |

> **Why Railway for the trading bot?**
> Bitget's API is protected by Cloudflare's WAF, which blocks authenticated requests from GitHub Actions' shared runner IP ranges. Railway uses different IP ranges that are not blocked. The research bot stays on GitHub Actions because it only makes unauthenticated market data calls.
...
**Usage:** Research and monitoring workflows use approximately 200–400 Actions minutes per month — well within the free limit. The trading bot runs on Railway (not GitHub Actions) to avoid Cloudflare IP blocks on Bitget's API.
...
### Railway — Required for Trading Bot ✅ 💰

The trading bot must run on Railway. Bitget's API is protected by Cloudflare, which blocks authenticated requests from GitHub Actions' shared IP ranges. Railway uses clean IP ranges that pass through without issue.

| Plan | Cost | Notes |
|---|---|---|
| Hobby | $5/month | 512MB RAM, always-on — more than enough for the bot |
| Pro | $20/month | Not needed |

**Setup:** Create a Railway account, deploy from your GitHub repo, and add your API keys as environment variables. The `railway.json` config file in the repo handles everything else automatically.
...
| Railway | ✅ Yes (trading bot hosting) | $5 |
