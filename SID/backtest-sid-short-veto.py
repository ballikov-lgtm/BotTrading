...
# EXPERIMENT (2026-06-13): veto SHORT entries on long-term-bullish assets.
#   none    — baseline v2.2.1 (no veto)
#   bullish — skip a SHORT entry when is_lt_bullish (weekly EMA50>EMA200 AND
#             weekly MACD>0 AND price>weekly SMA200). Tests whether avoiding
#             the UNH-class counter-trend short helps net, or also kills the
#             profitable shorts. The long-term-bullish classifier is computed
#             unconditionally below so this veto works on non-hybrid runs.
SHORT_VETO = os.environ.get('SID_SHORT_VETO', 'none').lower()
...
if SHORT_VETO != 'none':     _suffix_parts.append(f'shortveto-{SHORT_VETO}')
_suffix        = ('-' + '-'.join(_suffix_parts)) if _suffix_parts else ''
_vault         = Path(__file__).parent / 'strategy-test-vault' / 'short-veto-experiment'
_vault.mkdir(parents=True, exist_ok=True)
REPORT_MD      = _vault / f'backtest-short-veto-report{_suffix}.md'
REPORT_JSON    = _vault / f'backtest-short-veto-report{_suffix}.json'
...
def backtest_ticker_v2_1(ticker, df, variant, earnings_dates,
    ...
    # ── Instrumentation for the recovery-from-oversold short-veto SWEEP ──────
    # For every bar, record the weekly RSI level now + the MIN weekly RSI over
    # 8/13/26-week lookbacks (the washout depth). At a short entry these become
    # the trade's "recovery fingerprint": a deep recent washout (low min) + an
    # elevated current weekly RSI == shorting into a fresh recovery (the UNH
    # trap). Recorded per-trade so a post-processor can evaluate EVERY
    # (depth, lookback, recovery) threshold combination from one run.
    _weekly = df['Close'].resample('W-FRI').last().dropna()
    if len(_weekly) >= 30:
        _wr = wilder_rsi(_weekly, RSI_PERIOD)
        _wr_sh = _wr.copy(); _wr_sh.index = _wr_sh.index - pd.Timedelta(days=4)
        df['wk_rsi_now'] = _wr_sh.reindex(df.index, method='ffill')
        for _win in (8, 13, 26):
            _m = _wr.rolling(_win).min(); _m.index = _m.index - pd.Timedelta(days=4)
            df[f'wk_rsi_min{_win}'] = _m.reindex(df.index, method='ffill')
    else:
        df['wk_rsi_now'] = np.nan
        for _win in (8, 13, 26):
            df[f'wk_rsi_min{_win}'] = np.nan
    ...
    # Computed UNCONDITIONALLY (was `HYBRID_MODE and ...`) so the SHORT_VETO
    # experiment can read is_lt_bullish on non-hybrid runs. Harmless to baseline:
    # the column is only consumed by hybrid routing + the veto, both opt-in.
    if len(weekly_close) >= 200:
    ...
    for i, (date, row) in enumerate(df.iterrows()):
        ...
        if arm_dir == 'LONG' and in_pos is None:
            ...
            if rsi_falling and macd_falling and nogo_ok and weekly_ok:
                ...
                ep = row['Close']
                is_lt_bullish = bool(row.get('is_lt_bullish', False))
                # SHORT_VETO: skip counter-trend shorts on long-term-bullish names
                # (the UNH-class trap — short into a recovery). Disarm + continue.
                if SHORT_VETO == 'bullish' and is_lt_bullish:
                    arm_dir = None
                    prev_rsi, prev_macd, prev_signal = rsi, m, s
                    continue
                if stop > ep:
                    ...
                    in_pos = {
                        ...
                        'use_dynamic_tp':          use_dynamic,
                        'exit_mode':               'v2.1' if use_dynamic else 'v2',
                        # recovery fingerprint for the short-veto sweep
                        'wk_rsi_now_at_entry':   round(float(row.get('wk_rsi_now',   float('nan'))), 2),
                        'wk_rsi_min8_at_entry':  round(float(row.get('wk_rsi_min8',  float('nan'))), 2),
                        'wk_rsi_min13_at_entry': round(float(row.get('wk_rsi_min13', float('nan'))), 2),
                        'wk_rsi_min26_at_entry': round(float(row.get('wk_rsi_min26', float('nan'))), 2),
                    }
                    arm_dir = None
