"""
backtest-sid-v2.2.py — SID V2.2 Method backtest (INTRADAY-TOUCH TP1 + TP2)

Same entry rules as v2.1 / v2-weekly-or. The only difference vs the v2.1
backtest is the TP1 trigger semantics:

  V2.1 (close-based, baseline):
    TP1 fires on the daily CLOSE where RSI(14) >= 50 (long) or <= 50 (short).
    Fill price = bar's Close.

  V2.2 (intraday touch, this file):
    Before each bar, compute the price the next bar must print for RSI(14)
    to equal 50 using rsi_target_price() (mirrors SID/rsi-target-price.js).
    TP1 fires if THIS bar's High (long) / Low (short) REACHES that target
    intraday — simulating a resting GTC limit order on Alpaca.
    Fill price = the target itself (proxy for limit fill).

Everything else identical to v2.1:
  - Entry stack: v2-weekly-or (RSI cross + RSI(3) confirm + MACD direction
    + no-go zone + weekly RSI/MACD alignment + earnings/PPI blackout)
  - TP1 partial: 50% of position
  - Post-TP1: stop migrates to break-even on runner
  - TP2: whichever fires first — BE stop / SMA50 touch / SMA200 touch / timeout

GATE GOAL of this backtest (Step 19 of brief):
  Verify v2.2 win rate / profit factor / total PnL / avg-bars-held holds up
  or improves vs v2.1 baseline (69.5% WR / +$28k / 5y on AUTO tier, $200 fixed).
  v2.2 should win more often by catching the RSI-50 wicks v2.1 would miss
  (price touches 50 intraday then reverses → v2.1 round-trips into stop, v2.2
  locks the partial). The risk is wick-fills firing TP1 too early on choppy
  ranges where the daily close eventually clears 50 anyway — those would
  show up as identical TP1 hits at marginally worse prices.

Usage:
  cd SID
  python backtest-sid-v2.2.py

  SID_UNIVERSE=tier1 SID_BACKTEST_YEARS=5 python backtest-sid-v2.2.py
"""
...
_vault         = Path(__file__).parent / 'strategy-test-vault' / 'v2.2-intraday-tp1'
_vault.mkdir(parents=True, exist_ok=True)
REPORT_MD      = _vault / f'backtest-v2.2-validation-report{_suffix}.md'
REPORT_JSON    = _vault / f'backtest-v2.2-validation-report{_suffix}.json'
            ...
            if not in_pos['tp1_hit']:
                if side == 'LONG':
                    if row['Low'] <= in_pos['stop']:
                        ...
                        # V2.2 INTRADAY-TOUCH MODEL — TP1 fires when the bar's
                        # range reaches the RSI-50 target price computed from
                        # CLOSES THROUGH THE PRIOR BAR. The fill price proxy is
                        # the target itself (simulating a resting GTC limit on
                        # Alpaca that fills the instant the level is touched).
                        ...
                        tp1_touched = (
                            ...
                            and target_px > 0
                            and row['High'] >= target_px
                        )
                        if tp1_touched:
                            ex = float(target_px)
                            if not in_pos.get('use_dynamic_tp', True):
                                # HYBRID: non-bullish → V2-style FULL close at RSI 50 (touch)
                                ...
                                # V2.2: close 50%, mark, move stop to break-even, runner continues
