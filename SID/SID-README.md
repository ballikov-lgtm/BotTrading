...
## PDT version vs PDT-safe mode (v2.4.0)

SID ships with two execution modes. They trade identically at the **signal** level — same entries, same TP1/TP2/stop **levels**. They differ only in **how orders are placed at the broker**, which is what determines whether a position can round-trip on the same day (and therefore whether the Pattern Day Trader rule can bite).

**The Pattern Day Trader (PDT) rule — the plain facts.** A "day trade" is opening and closing the same security on the same day. If a **margin** account makes **4 or more day trades in 5 business days**, FINRA/Alpaca flag it a Pattern Day Trader and require it to hold **$25,000+** in equity; dropping below can get the account **restricted from day trading** until it's topped back up. This applies **only to margin accounts** — but SID takes **short** trades, and shorting **requires** a margin account, so a **cash account can't run SID at all**, and any account that can is margin and is therefore PDT-subject.

| | **PDT version** (standard, default) | **PDT-safe mode** (`SID_PDT_SAFE=true`) |
|---|---|---|
| **Broker orders** | Real GTC stop + resting intraday short-TP1 limit from entry | **None** — no stop, no resting TP1 limit |
| **How exits fire** | Broker orders can fill **intraday**, same day as entry | Only the **once-a-day** bot poll; earliest exit is the **next day's open** |
| **PDT exposure** | Can create same-day round trips → PDT-subject | **Cannot** open+close same day → **PDT-immune** |
| **Best for** | Accounts holding **$25,000+** at all times | Accounts **under $25,000** |
| **Intraday protection** | Yes — a broker stop sits at the exchange | **No** — a hard mid-day move isn't cut until the next run's open |
| **Profitability** | Best fills (baseline) | **Modest few-percent** reduction vs the PDT version |
| **Missed-run risk** | Broker safety net still resting | Position unmanaged longer (no broker net) |
...
| v2.4.2 | 1 Sep 2026 | **Shorts-TP1 OCO fix (broker-order plumbing — no signal-logic change).** A v2.2 **short** had no working intraday take-profit: `maintainV2_2BrokerOrders` left the full-size entry stop holding **all** shares — it cancelled `existingStop.id` from a suffix scan that used `endsWith('-stop')` and so **missed the timestamped re-placed stops** (`${prefix}-stop-${ts}`, the twin of the v2.2.6 `closePosition` bug), and used a fixed 800 ms sleep instead of waiting for share release — so the runner stop **and** the TP1 OCO both failed `insufficient qty available: 0` (only `console.warn`'d). Because `checkPositions` Branch A **skips the daily-poll RSI-50 TP1 for v2.2 shorts**, the short had **no working TP1 at all** (live: PYPL 17-sh short, RSI ~32.9 ≪ 50, stuck 100% open for days). Fix: (1) `findOpenOrder` now matches `<suffix>(-\d+)?$` so bare **and** timestamped `-stop`/`-tp1` are found (`-restop-<ts>` is intentionally excluded — cancelled via the tracked `brokerStopOrderId`); (2) the maintain stop reset cancels the **tracked** `pos.brokerStopOrderId` **by id** + the suffix-found stop + (pre-TP1 short) any resting TP1 OCO, then **waits for share release** (`waitForNoOpenOrders`, not a sleep), then places the **runner-only stop** (`shares_total − tp1`), resetting only on a missing/wrong-size/wrong-price stop (no steady-state churn); (3) the TP1 half is placed as the OCO **buy-limit at the RSI-50 target** with its shares now free — where price has already passed the target the buy-limit is **marketable → fills immediately → banks TP1**. Also fixes the same regex miss's latent twins (short TP1 OCO re-price/dedup; long post-TP1 BE-stop maintenance). (4) **Symbol-scoped sweep before any close/reset** (new `executor.cancelAllOpenOrders`) cancels EVERY open order on the one symbol — including a **MANUAL** break-even stop the user placed via the Alpaca UI (Alpaca-generated `client_order_id`, no SID tag → invisible to the tracked-id + suffix cancels) that would otherwise hold the shares and hang `waitForNoOpenOrders`; wired into maintain's reset AND both close branches (TP1/TP2); never touches another symbol. Ships with the **cron-redundancy** `sid.yml` change (3 staggered weekday runs at 14:35 / 16:35 / 18:35 UTC so a delayed GitHub Actions run can't miss the market window). Tested: 26/26 logic assertions (regex incl. manual-order exclusion, PYPL 17→9+8 reset, marketable OCO @ RSI-50 target, symbol-scoped sweep of a SID + a manual order while sparing other symbols, manual-BE-on-a-runner, no-churn / long regressions). **NO canon rule change** — RSI 30/70, MACD alignment, the RSI-50 TP1 trigger, TP2 SMA50/200/BE/timeout, the share math, and the v2.2.5/2.2.6 cancel-first close paths are all byte-for-byte unchanged. |
| v2.4.1 | 18 Jul 2026 | **Accounting reconciled to Alpaca (accounting-correctness patch — no signal-logic change).** The dashboard's live net-P&L headline now shows the **real Alpaca paper-account equity** (`+$182.18` on a `$100,000` base) from `alpaca-account-sid.json`, labelled **"Paper account (simulated)"**, instead of a sum of the internal records that had drifted from the broker. Two drift layers were fixed: (1) the ledger's **running `realizedPnl` counter** was never re-derived when records were corrected, so a paper trade (MCD) removed from the records left a **phantom −$78.14** + inflated trade count; (2) each close booked a **simulated price** (TP1 at the RSI-50 bar close, TP2 at the historical SMA-touch) not the **real Alpaca fill**, understating profit on records that closed days later. A read-only cloud tool (`reconcile-account.js`) matched every closed position to its **real Alpaca fills** (FIFO across same-symbol positions, searching exits through *now* so late runner exits are caught), **reconstructed the dropped MCD paper trade** from the broker's fill history, and re-derived the ledger — reconciling the paper books **to the dollar** (`+$182.37` internal vs `+$182.18` Alpaca; `$0.19` fees/rounding). Going forward the bot **books each close at the confirmed Alpaca `filled_avg_price`** (TP1 Branch A, TP2 Branch B, `recordExternalClose`), **stamps every record `pnl_source`** (`alpaca_fill`/`estimate`) + `mode`, and **re-derives the ledger from records on every close** (never a running counter → phantom-proof). **Paper/live segregated:** the ledger sums only the current mode's records; a future live ledger starts fresh (`mode:live`) and never mixes with paper or the live-only tax report. New: `reconcile-account.js`, `alpaca-account-sid.json`, `.github/workflows/sid-reconcile.yml` (dry-run/write), `alpaca-client.js` `getActivities()`/`getPortfolioHistory()`. **NO canon rule change** — the RSI-50 TP1 trigger, the SMA50/200/BE/timeout TP2 exits, the share math, the cancel-first stop logic and the entry/scan logic are byte-for-byte unchanged; only the exit-price *source* and the ledger *derivation* changed, so the headline backtest numbers are unchanged. |
| v2.4.0 | 17 Jul 2026 | **PDT-safe execution mode (new opt-in execution mode — no signal-logic change).** The standard bot places real broker GTC stops + a resting intraday short-TP1 limit from entry, which *can* fill the **same day** as entry — a day trade. A **margin** account under **$25,000** making 4+ day trades in 5 business days is flagged a **Pattern Day Trader** by FINRA/Alpaca and can be **restricted from day trading**; SID shorts require margin, so sub-$25K SID accounts are PDT-subject (a cash account can't run SID at all). v2.4.0 restores v2.0's PDT-immune design as an **opt-in** mode: `SID_PDT_SAFE=true` (env, **default FALSE**). When on, the bot places **no broker order that can fire intraday same-day** — it **skips `maintainV2_2BrokerOrders` entirely** (no resting GTC stop, no resting short-TP1 limit) and does **not** place the entry-time stop, on **both** the normal entry path (`executor.openEntryNoStop()`) and the approved-entry path (`approve-trade.js`). PDT-safe positions carry `pdt_safe:true` + `brokerStopOrderId:null` → `isV2_2Position()` returns **false** (guarded explicitly), so they're managed **solely by the daily-poll** in `checkPositions` (TP1 RSI-50, TP2 SMA50/SMA200/BE/timeout, and a stop breach → market close at the **next run's open**). **Same-day guarantee:** `checkPositions` runs *before* the entry loop adds the new position (so it isn't even scanned on entry day) and, once scanned on the next run, only bars with `date` **strictly >** `openDate` can trigger a close; with no broker orders, `syncPositions` can't detect a same-day fill either — so the earliest possible exit is always a **later calendar day**. **Trade-offs (inherent to PDT-immunity):** no intraday stop → a hard mid-day move isn't cut until the next run's open (gap/slippage) and a **modest few-percent** profit reduction vs the PDT version; a missed run leaves a position unmanaged longer (no broker safety net). **Standard mode (`pdtSafe=false`) is byte-for-byte unchanged.** The fresh installer (`SID-DEPLOY-PROMPT.md` Step 8a) now asks your funding size and recommends the PDT version ($25K+, `SID_PDT_SAFE=false`) or PDT-safe (<$25K, `SID_PDT_SAFE=true`), explains the PDT rule + the cash-vs-margin fact + a not-financial-advice caveat. `sid.yml` + `sid-approve-trade.yml` pass `SID_PDT_SAFE` through. See [PDT version vs PDT-safe mode](#pdt-version-vs-pdt-safe-mode-v240). **NO canon rule change** — RSI 30/70, RSI+MACD alignment, the RSI-50 TP1 trigger, the 14-day earnings blackout and the AUTO-tier 80-ticker universe are all untouched, so the headline backtest numbers are unchanged. |
...
| `HOW-TO-FUND-ALPACA.md` | **How to Fund an Alpaca Account (Bank Transfer)** — the cheapest + safest way to move money from the UK into a live Alpaca account (Wise/Revolut USD account → free ACH), with a safety checklist and the tax / W-8BEN note. Read this before going live |
| `HOW-TO-TAX-REPORT.md` | **Offline tax report (no coding)** — the dead-simple guide: double-click one launcher to build an Excel of all your closed trades, on YOUR machine, that you filter by UK tax year and hand to your accountant. Nothing is uploaded; the file never leaves your computer |
| `tax-report.py` | Builds the interactive tax Excel from `closed-positions-sid.json` — one row per closed trade, AutoFilter + a filter-aware `SUBTOTAL(109)` realised-P&L total, and a computed **Month** + **UK Tax Year** (6 Apr–5 Apr) column. Output goes to `SID/tax-reports/` (git-ignored — never committed) |
| `GENERATE-TAX-REPORT.bat` / `generate-tax-report.command` | **Double-click launchers** (Windows / Mac) for the tax report — refresh latest trades, run `tax-report.py`, open the Excel. No command line needed |
