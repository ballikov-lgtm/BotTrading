"""
backtest-sid-v2.2.1-hybrid.py — SID V2.2.1 HYBRID (long-close / short-intraday)

Refinement of v2.2 motivated by the per-side breakdown of the v2.2 backtest:
  - SHORTS: v2.2 intraday touch was strictly better than v2.1 close --- higher
    WR AND higher total PnL. The bullish drift of US equities punishes any
    short that round-trips through RSI 50; intraday-touch protects.
  - LONGS: v2.2 had higher WR (+4.6pp) but lower total PnL (-$2,329) than v2.1
    because the bullish drift WORKS FOR longs --- letting price close past
    RSI 50 books a bigger TP1 partial than locking at exactly 50.

v2.2.1 routes by side:
  - LONG positions:  v2.1 close-based TP1 (RSI >= 50 on daily CLOSE)
                     Fill price = bar's Close.
  - SHORT positions: v2.2 intraday-touch TP1 (resting GTC limit at rsi_target_price)
                     Fill price = the target itself (proxy for limit fill).

Everything else identical to v2.1 / v2.2:
  - Entry stack: v2-weekly-or
  - TP1 partial: 50% of position
  - Post-TP1: stop migrates to break-even on runner
  - TP2: BE stop / SMA50 touch / SMA200 touch / timeout

GATE GOAL of this backtest:
  Confirm projected hybrid metrics from the per-side decomposition:
     - WR  >= pure v2.1 WR (69.6%)     -> projection: ~71.4%
     - PnL >= pure v2.1 PnL ($29,317)  -> projection: ~$29,694
     - Net trade-level dominance over pure v2.1 (every metric >=)
  If gate clears, this is the live SID/Pine/bot config --- pure v2.2 retires.

Usage:
  cd SID
  python backtest-sid-v2.2.1-hybrid.py

  SID_UNIVERSE=tier1 SID_BACKTEST_YEARS=5 python backtest-sid-v2.2.1-hybrid.py
"""
...
# EXPERIMENT (2026-06-28): replicate the LIVE BOT's looser entry to quantify the
# cost of its drift from the validated strategy. When SID_BOT_PARITY=true the arm:
#   - drops the RSI(3) extreme confirmation        (bot detectEntrySignal has none)
#   - drops the weekly SMA50>SMA200 arm gate        (bot has none)
#   - replaces the 3-day arm timeout with the bot's "episode" model: the arm
#     stays valid until RSI recovers to 50 (bot's oversoldIdx-until-RSI>=50 logic)
# Everything else (no-go zone, weekly-direction trigger, TP1/TP2) stays identical,
# so the delta vs baseline isolates exactly the three entry-gate divergences.
BOT_PARITY          = os.environ.get('SID_BOT_PARITY', 'false').lower() in ('true', '1', 'yes')
# Re-arm cooldown test (Alan, 2026-06-28): after a 3-day arm expires WITHOUT an
# entry, lock the asset out of re-arming for this many trading days, then release
# it. 0 = the validated model's free re-arm (re-arm next bar while still oversold).
# 5 = Alan's variant: arm 3d -> expire -> 5d lockout -> released -> can arm again.
REARM_COOLDOWN_DAYS = int(os.environ.get('SID_REARM_COOLDOWN', '0'))
# Side-specific re-arm cooldown (Alan, 2026-06-28): apply the lockout to shorts
# only, leaving longs on free re-arm. Rationale: bullish names recover in a V,
# so a fast long re-arm catches the snapback; shorts benefit from filtering the
# low-quality counter-trend repeats. Default each side to the symmetric value.
REARM_COOLDOWN_LONG  = int(os.environ.get('SID_REARM_COOLDOWN_LONG',  str(REARM_COOLDOWN_DAYS)))
REARM_COOLDOWN_SHORT = int(os.environ.get('SID_REARM_COOLDOWN_SHORT', str(REARM_COOLDOWN_DAYS)))
...
if BOT_PARITY:               _suffix_parts.append('botparity')
if REARM_COOLDOWN_LONG != REARM_COOLDOWN_SHORT:
    _suffix_parts.append(f'rearmcdL{REARM_COOLDOWN_LONG}S{REARM_COOLDOWN_SHORT}')
elif REARM_COOLDOWN_SHORT != 0:
    _suffix_parts.append(f'rearmcd{REARM_COOLDOWN_SHORT}')
_suffix        = ('-' + '-'.join(_suffix_parts)) if _suffix_parts else ''
_vault         = Path(__file__).parent / 'strategy-test-vault' / 'bot-parity-experiment'
_vault.mkdir(parents=True, exist_ok=True)
REPORT_MD      = _vault / f'backtest-bot-parity-report{_suffix}.md'
REPORT_JSON    = _vault / f'backtest-bot-parity-report{_suffix}.json'
...
VARIANTS = [
    {
        'name': 'v2.2-intraday-tp1',
        'desc': 'V2.2 = v2-weekly-or entry + INTRADAY-TOUCH RSI-50 TP1 + TP2 partial exits (50/50)',
...
def rsi_target_price(closes, period=14, target=50):
    """Solve for the next bar's close that would make Wilder RSI(period) == target.

    Mirrors SID/rsi-target-price.js exactly so backtest math matches live bot.
    Used by V2.2's intraday-touch TP1 model: compute the level for the next bar,
    then check whether the bar's high (long) / low (short) reached it.

    Args:
        closes: chronological closing prices (list or pandas Series of floats)
        period: RSI lookback (default 14)
        target: RSI level to solve for (default 50)
    Returns:
        Target price (float, 2dp) or None if not enough data.
    """
    ...
    if avg_loss == 0 and avg_gain == 0:
        return None  # Flat history — undefined

    prev_close = float(closes[-1])
    p1 = period - 1
    k = target / 100.0

    # "Price up" branch (diff >= 0)
    up = ((k * avg_loss - (1 - k) * avg_gain) * p1) / (1 - k)
    if up >= 0:
        return round(prev_close + up, 2)

    # "Price down" branch (diff < 0)
    L = (avg_gain * p1 * (1 - k) - k * avg_loss * p1) / k
    return round(prev_close - L, 2)
...
def backtest_ticker_v2_1(ticker, df, variant, earnings_dates,
    ...
    rearm_cooldown = 0   # trading-day lockout after an arm expires (Alan's re-arm test)
    cooldown_side  = None  # which side ('LONG'/'SHORT') the active lockout applies to
    in_pos = None
    prev_rsi  = None
    prev_macd = None
    prev_signal = None
    ...
    for i, (date, row) in enumerate(df.iterrows()):
            ...
            if not in_pos['tp1_hit']:
                if side == 'LONG':
                    if row['Low'] <= in_pos['stop']:
                        ...
                        # V2.2.1 HYBRID --- LONGS use V2.1 close-based check.
                        # In a long-term bullish market, the bullish drift
                        # WORKS FOR longs: letting price close past RSI 50
                        # books a bigger TP1 partial than locking exactly at 50.
                        # (Shorts use the v2.2 intraday-touch model below for
                        # protection against round-trips against the drift.)
                        # `rsi` is the local variable extracted from row['RSI']
                        # at the top of the bar-iteration loop above.
                        if not pd.isna(rsi) and rsi >= RSI_EXIT_TP1:
                            ex = row['Close']
                            if not in_pos.get('use_dynamic_tp', True):
                                # HYBRID-CLASSIFY: non-bullish -> V2-style FULL close at RSI 50
                                ...
                                # V2.1: close 50% at close, mark, move stop to BE
                    ...
                    if row['High'] >= in_pos['stop']:
                        ...
                        # V2.2 INTRADAY-TOUCH MODEL (SHORT) — fires when bar's
                        # LOW reaches the RSI-50 target from below. Target
                        # solves for "what price falls would push RSI to 50?"
                        closes_prior = df['Close'].iloc[:i].values
                        target_px = rsi_target_price(closes_prior, RSI_PERIOD, RSI_EXIT_TP1)
                        tp1_touched = (
                            target_px is not None
                            and not pd.isna(target_px)
                            and target_px > 0
                            and row['Low'] <= target_px
                        )
                        if tp1_touched:
                            ex = float(target_px)
                            if not in_pos.get('use_dynamic_tp', True):
                                # HYBRID: shorts always V2-style FULL close at RSI 50 (touch)
                                pnl_full = (entry - ex) * shares_total
                                in_pos['tp1_exit_px'] = ex
                                in_pos['tp1_exit_date'] = date.strftime('%Y-%m-%d')
                                in_pos['tp1_reason'] = 'rsi50_intraday'
                                in_pos['tp1_shares'] = shares_total
                                in_pos['tp1_pnl'] = round(pnl_full, 2)
                                in_pos['tp2_shares'] = 0
                                in_pos['tp2_pnl'] = 0.0
        ...
        # Re-arm cooldown countdown: tick down each disarmed bar until the asset
        # is "released" and allowed to arm again (Alan's lockout test).
        if arm_dir is None and rearm_cooldown > 0:
            rearm_cooldown -= 1
            if rearm_cooldown == 0:
                cooldown_side = None

        # BOT_PARITY drops the RSI(3) + weekly-SMA arm gates (the live bot has neither).
        arm_long_gate  = (BOT_PARITY or (rsi3_ok_long  and wk_long))
        arm_short_gate = (BOT_PARITY or (rsi3_ok_short and wk_short))
        # Side-specific lockout: a short's cooldown blocks only re-shorting; longs free.
        long_blocked   = (rearm_cooldown > 0 and cooldown_side == 'LONG')
        short_blocked  = (rearm_cooldown > 0 and cooldown_side == 'SHORT')
        if arm_dir is None and in_pos is None and not earn_block:
            if allow_longs and rsi < RSI_OVERSOLD and arm_long_gate and not long_blocked:
                arm_dir = 'LONG'
                arm_low = row['Low']; arm_high = row['High']
                days_since_arm = 0
            elif allow_shorts and rsi > rsi_overbought and arm_short_gate and not short_blocked:
                arm_dir = 'SHORT'
                arm_low = row['Low']; arm_high = row['High']
                days_since_arm = 0
            ...
            if BOT_PARITY:
                # Bot's episode model: arm stays valid until RSI recovers to 50.
                expired = (arm_dir == 'LONG' and rsi >= 50) or (arm_dir == 'SHORT' and rsi <= 50) or earn_block
            else:
                expired = days_since_arm > TIMEOUT_DAYS_ARM or earn_block
            if expired:
                cd = REARM_COOLDOWN_LONG if arm_dir == 'LONG' else REARM_COOLDOWN_SHORT
                if cd > 0:
                    rearm_cooldown = cd
                    cooldown_side = arm_dir
                arm_dir = None
...
def main():
    ...
    for idx, ticker in enumerate(universe, 1):
        try:
            ...
            wr = (sum(1 for t in trades if t['tp1_reason'] in ('rsi50', 'rsi50_intraday')) / len(trades)) if trades else 0.0
    ...
    df_t['win']  = df_t['tp1_reason'].isin(['rsi50', 'rsi50_intraday'])  # TP1 hit = winning trade
