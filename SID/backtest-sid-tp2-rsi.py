...
# EXPERIMENT (2026-06-13): TP2 trigger mode for the runner.
#   baseline    — first of {BE, SMA50, SMA200, timeout}        (current v2.2.1)
#   rsi_replace — first of {BE, RSI-extreme, timeout}          (Alan's proposal — drop SMAs)
#   rsi_add     — first of {BE, RSI-extreme, SMA50, SMA200, timeout}  (RSI as an extra early exit)
# RSI-extreme = RSI(14) >= overbought for LONG runner / <= oversold for SHORT runner.
TP2_MODE = os.environ.get('SID_TP2_MODE', 'baseline').lower()
...
if TP2_MODE != 'baseline':   _suffix_parts.append(f'tp2-{TP2_MODE}')
_suffix        = ('-' + '-'.join(_suffix_parts)) if _suffix_parts else ''
_vault         = Path(__file__).parent / 'strategy-test-vault' / 'tp2-rsi-experiment'
_vault.mkdir(parents=True, exist_ok=True)
REPORT_MD      = _vault / f'backtest-tp2-rsi-report{_suffix}.md'
REPORT_JSON    = _vault / f'backtest-tp2-rsi-report{_suffix}.json'
    ...
    for i, (date, row) in enumerate(df.iterrows()):
        ...
        if in_pos is not None:
            ...
            # Branch B: TP1 already hit, running on remainder with break-even stop.
                ...
                # RSI-extreme gate (close-based, so exit at this bar's Close).
                # LONG runner exhausts at overbought; SHORT runner at oversold.
                rsi_extreme_long  = TP2_MODE in ('rsi_replace', 'rsi_add') and (not pd.isna(rsi)) and rsi >= rsi_overbought
                rsi_extreme_short = TP2_MODE in ('rsi_replace', 'rsi_add') and (not pd.isna(rsi)) and rsi <= RSI_OVERSOLD
                use_sma = TP2_MODE != 'rsi_replace'   # rsi_replace drops the SMA touches

                if side == 'LONG':
                    # Break-even stop on remaining 50%
                    if row['Low'] <= in_pos['stop']:
                        tp2_close = True; tp2_reason = 'breakeven_stop'; tp2_exit_px = in_pos['stop']
                    # RSI overbought — runner momentum exhausted
                    elif rsi_extreme_long:
                        tp2_close = True; tp2_reason = 'rsi_overbought'; tp2_exit_px = row['Close']
                    # 50d MA touched from above (price coming back down to it)
                    elif use_sma and ma50 is not None and row['Low'] <= ma50 <= row['High']:
                        tp2_close = True; tp2_reason = 'sma50_touch';  tp2_exit_px = ma50
                    # 200d MA touched
                    elif use_sma and ma200 is not None and row['Low'] <= ma200 <= row['High']:
                    ...
                    if row['High'] >= in_pos['stop']:
                        ...
                        tp2_close = True; tp2_reason = 'rsi_oversold';  tp2_exit_px = row['Close']
                    elif use_sma and ma50 is not None and row['Low'] <= ma50 <= row['High']:
                        tp2_close = True; tp2_reason = 'sma50_touch';  tp2_exit_px = ma50
                    elif use_sma and ma200 is not None and row['Low'] <= ma200 <= row['High']:
                        tp2_close = True; tp2_reason = 'sma200_touch'; tp2_exit_px = ma200
